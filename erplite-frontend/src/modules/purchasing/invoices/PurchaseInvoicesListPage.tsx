/**
 * src/modules/purchasing/invoices/PurchaseInvoicesListPage.tsx
 *
 * Read-only list of purchase invoices (purchasing.purchase_invoice table).
 */
...
export function PurchaseInvoicesListPage() {
  ...
  const { data: invoices, isLoading, error } = useQuery({
    ...
    queryFn: () => reportingApi.listPurchaseInvoices({ limit: 200 }),
  })
  ...
  return (
    <div>
      ...
      {isLoading && (
        <Table>
          <thead>
            <tr><Th>رقم الفاتورة</Th><Th>المورد</Th><Th>التاريخ</Th><Th>المبلغ</Th><Th>المدفوع</Th><Th>المستحق</Th><Th>الحالة</Th></tr>
          </thead>
          <tbody>
      ...
      {invoices && (
        <>
          <CountSummary
            ...
            breakdown={[
              { label: 'إجمالي القيمة', count: Math.round(totalAmount), tone: 'amber' },
              { label: 'إجمالي المستحق', count: Math.round(totalDue), tone: 'rose' },
            ]}
          />
          {filtered.length === 0 ? (
            <Table>
              <thead><tr><Th>رقم الفاتورة</Th><Th>المورد</Th><Th>التاريخ</Th><Th>المبلغ</Th><Th>المدفوع</Th><Th>المستحق</Th><Th>الحالة</Th></tr></thead>
              <tbody><tr><td colSpan={7}><EmptyState message="لا توجد فواتير مشتريات بعد — حالياً يتم إنشاء الفواتير عبر قاعدة البيانات مباشرة" /></td></tr></tbody>
            </Table>
          ) : (
            <Table>
              <thead>
              ...
              <tbody className="divide-y">
                {filtered.map((inv) => (
                  <tr key={inv.uuid} className="hover:bg-gray-50">
                    ...
                    <Td>
                      <Link to={`/purchasing/suppliers/${inv.supplier_uuid}`} className="text-blue-600 hover:underline">
                        {inv.supplier_name}
                      </Link>
                    </Td>
