/**
 * src/modules/purchasing/orders/PurchaseOrdersListPage.tsx
 *
 * Mirrors SalesOrdersListPage. Adds client-side status filter + count
...
export function PurchaseOrdersListPage() {
  ...
  const { data: orders, isLoading, error } = useQuery({
    queryKey: ['purchase-orders', search || undefined],
    queryFn: () => purchasingApi.listPurchaseOrders(search || undefined),
  })
  ...
  const { sortKey, sortDir, toggleSort, sortData } = useSort<PurchaseOrderSummary>('document_date', 'desc')
  const sorted = sortData(filtered, (o) => o[sortKey as keyof PurchaseOrderSummary] ?? '')
          ...
          {sorted.length === 0 ? (
            <Table>
              ...
              <tbody>
                <tr>
                  <td colSpan={3}>
                    <EmptyState message={hasActiveFilters ? 'لا توجد أوامر مطابقة' : 'لا توجد أوامر شراء بعد'} />
                  </td>
                </tr>
              </tbody>
            </Table>
              ...
              <tbody className="divide-y">
                {sorted.map((o) => (
                  <tr
                    ...
                    onClick={() => navigate(`/purchasing/orders/${o.uuid}`)}
                  >
